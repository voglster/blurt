...
class Config:
    ...
    def sample_rate(self) -> int:
        """Audio sample rate."""
        return int(self._config["audio"]["sample_rate"])

    @property
    def hold_threshold_ms(self) -> int:
        """Tab hold threshold in milliseconds."""
        return int(self._config["audio"]["hold_threshold_ms"])

    @property
    def post_release_ms(self) -> int:
        """Post-release recording time in milliseconds."""
        return int(self._config["audio"]["post_release_ms"])

    @property
    def channels(self) -> int:
        """Audio channels."""
        return int(self._config["audio"]["channels"])
    ...
    def log_level(self) -> str:
        """Logging level."""
        return str(self._config["daemon"]["log_level"])

    @property
    def typing_delay(self) -> float:
        """Delay between typed characters."""
        return float(self._config["output"]["typing_delay"])
